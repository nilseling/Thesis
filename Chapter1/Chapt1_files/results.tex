...
To characterize the variation observed in \textbf{Fig.~\ref{fig1:all_cells}}, we dissected differences in gene expression between the two different mouse strains using naive CD4$^+$ T cells. Similar for \textbf{Fig.~\ref{fig1:all_cells}}, we found that CD4$^+$ T cells cluster by strain when only profiling naive cells \textbf{(Fig.~\ref{fig1:strain_specific}A)}.
